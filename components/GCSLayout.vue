    ...
    <div class="flex-1 flex flex-col md:flex-row overflow-hidden">
      ...
      <div class="flex-1 bg-black relative flex flex-col min-h-0">
        ...
        <div class="bg-gray-800 border-b border-gray-700">
          <nav class="flex space-x-2 sm:space-x-4 px-1.5 overflow-x-auto">
            <button
              v-for="tab in tabs"
              :key="tab.id"
              @click="activeTab = tab.id"
              class="py-1 px-2 text-[10px] sm:text-xs font-medium border-b-2 transition-colors whitespace-nowrap flex-shrink-0"
              :class="[
                activeTab === tab.id
            ...
            <button
              ...
              class="md:hidden py-1 px-2 text-[10px] sm:text-xs font-medium border-b-2 transition-colors whitespace-nowrap flex-shrink-0 ml-auto"
              :class="[
                showLEDPanel
        ...
        <div class="flex-1 relative min-h-0">
          <!-- Camera Tab -->
          <div v-if="activeTab === 'camera'" class="p-0.5 h-full">
      ...
      <div
        class="bg-gray-700 text-white p-1.5 transition-all duration-300 overflow-y-auto"
        :class="[
          showLEDPanel ? 'block' : 'hidden md:block',
          'w-full md:w-[180px] lg:w-[200px]',
          'max-h-[40vh] md:max-h-none'
        ]"
      >
        <h3 class="text-xs font-bold mb-1">DEXI LED Ring</h3>
